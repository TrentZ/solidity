	...
	virtual void operator()(Block& _block)
	{
		util::iterateReplacing(
			...
			[&](Statement& _statement) -> std::optional<vector<Statement>> {
				if (
					auto assignment = std::get_if<Assignment>(&_statement);
					assignment && m_currentFunctionMemorySlots && util::contains_if(assignment->variableNames, [&](auto const& var){
						return m_currentFunctionMemorySlots->count(var.name);
					})
				)
				{
					auto loc = locationOf(_statement);
					...
					auto varDecl = std::get_if<VariableDeclaration>(&_statement);
					varDecl && m_currentFunctionMemorySlots && util::contains_if(varDecl->variables, [&](auto const& var) {
						return m_currentFunctionMemorySlots->count(var.name);
					})
				)
				{
